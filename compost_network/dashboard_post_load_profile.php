...
$(document).ready(function(){
 ...
 var postData ="<?php echo $request_userid; ?>";
        ...
        if(postRow <= postCount){
            ...
            $.ajax({
                url: 'post_loadmoreData_profile.php',
                type: 'post',
...
$status_result = $db->prepare("SELECT count(*) as allcount FROM posts where userid=:userid");
$status_result->execute(array(':userid' =>$request_userid));
...
$result->bindValue(':userid', (int) trim($request_userid), PDO::PARAM_INT);
//$result->bindValue(':userid', $request_userid, PDO::PARAM_STRING);
...
if($count_post ==0){
echo "<div style='background:red;color:white;padding:10px;border:none;'>No Post updates Yet From This Member.. <b></b></div>";
}
